...
import { AjaxInterceptorsService } from '../../service/ajax-interceptors.service';
import { GeneralService } from '../../service/general.service';
@Component({
  selector: 'app-abcd-grid',
  templateUrl: './abcd-grid.component.html',
  styleUrl: './abcd-grid.component.scss',
  standalone: true,
  imports: [HttpClientModule, jqxGridModule],
})
export class AbcdGridComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    private ajaxService: AjaxInterceptorsService,
    private commos: GeneralService
  ) { }
    ...
    this.ajaxService
      ...
      .subscribe(
        (res) => {
          ...
          this.commos.griddata
          this.resData = this.commos.griddata;
          this.dataBindinginGrid(this.commos.griddata, this.direction);
        },
        (err) => { }
      );
  }
  ...
  dataBindinginGrid(res, lang) {
    ...
    this.actiongridDC = [];
    let datafromsessionDepesndsonrole = 'Company Admin'
    if (res['displayType'] == 'grid' || true) {
      this.actiongridDC = JSON.parse(this.data.jqdetails)[datafromsessionDepesndsonrole];
      this.columngroups = [
        {
          text:
            `Action`, align: 'center', name: 'action1'
        },
      ];
              ...
              cellsrenderer: (row): any => {
                if (JSON.parse(this.data['datavalues'])[row])
                  ...
                  <span
                    ...
                    data-label-off=${JSON.parse(this.data['datavalues'])[row].isactive
                      ? 'OFF'
                      : 'ON'
                    }
                    data-label-on=${JSON.parse(this.data['datavalues'])[row].isactive
                      ? 'ON'
                      : 'OFF'
                    }
                  >
          ...
          case 'archive':
            this.columns.push({
              ...
              cellsrenderer: (row, value, j, u, o, obj): string => {
                ...
                // return arctext;
                return `<span title="${obj['isarchived'] ? 'Unarchive' : 'Archive'
          ...
          case 'private':
            this.columns.push({
              ...
              cellsrenderer: (row, value, j, u, o, obj): string => {
                ...
                //   </svg></span>`;
                return `  <span title="${obj['isprivate'] ? 'Unfavorite' : 'Favorite'
                  }"> <i class="bi ${obj['isprivate'] ? 'bi-star-fill' : 'bi-star'
                  } "></i> </span>`; //*mark32
              },
              // buttonclick: (row): void => { this.getRowData(row); },
              cellclassname: 'cssclass', //mark3
  ...
  cellClickRow(e) {
    ...
    this.abcd_data_emit.emit(local);
  }
